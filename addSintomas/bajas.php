...
if (isset($_GET['nombre']) && !empty($_GET['nombre'])) {
    ...
    if ($stmt_buscar = mysqli_prepare($conexion, $sql_buscar)) {
        ...
        if (mysqli_num_rows($resultado) > 0) {
            ...
            $sql_borrar = "DELETE FROM registro_Sintomas WHERE nombre = ?";
            ...
            if ($stmt_borrar = mysqli_prepare($conexion, $sql_borrar)) {
                ...
                if (mysqli_stmt_execute($stmt_borrar)) {
                    // Éxito
                    echo "<script>
                            alert('El sintoma \"$nombre\" fue dada de baja exitosamente.');
                            window.location.href = '../login/intExpertoSintomas.php';
                          </script>";
                } else {
                    echo "<script>alert('Error al intentar borrar de la base de datos.'); window.location.href = '../login/intExpertoSintomas.php';</script>";
                }
