...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt_buscar = mysqli_prepare($conexion, $sql_buscar)) {
        ...
        if ($fila = mysqli_fetch_assoc($resultado)) {
            ...
            $sql_update = "UPDATE registro_enfermedades SET descripcion=?, ruta_imagen=? WHERE id=?";
            if ($stmt_update = mysqli_prepare($conexion, $sql_update)) {
                ...
                if (mysqli_stmt_execute($stmt_update)) {
                    ...
                    echo "<script>alert('Error al modificar en la base de datos.'); window.location.href='../login/intExperto.php';</script>";
                }
